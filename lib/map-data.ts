...
export async function getMapData(): Promise<CcaaMapItem[]> {
  const today = new Date().toISOString().split("T")[0];

  const rows = await db
    .select({ ccaaCode: funds.ccaaCode, closeDate: funds.closeDate })
    .from(funds)
    .where(
      and(
  ...
  for (const row of rows) {
    ...
    if (row.closeDate) {
      const diff = Math.ceil(
        (new Date(row.closeDate).getTime() - new Date(today).getTime()) / 86400000
      );
